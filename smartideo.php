...
define('SMARTIDEO_VERSION', '2.0.8');
...
class smartideo{
    ...
    public function smartideo_embed_handler_bilibili( $matches, $attr, $url, $rawattr ) {
        if ($this->option['bilibili_player']) {
            $embed = $this->get_iframe("http://www.bilibili.com/html/html5player.html?aid={$matches['video_id']}&page=1", $url);
        } else {
            if($this->is_https()){
